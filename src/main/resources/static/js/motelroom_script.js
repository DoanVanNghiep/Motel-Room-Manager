...
function getInfoForSearch() {
    if (request.readyState == 4) {
        ...
        document.getElementById("clientHomeResult").innerHTML = htmlSearchResult;
    }
}
// cái này dành cho search đơn hàng trong shipper
function activeAsAjaxShipper(link) {
    if (window.XMLHttpRequest) {
        request = new XMLHttpRequest();
    } else if (window.ActiveXObject) {
        request = new ActiveXObject("Microsoft.XMLHTTP")
    ...
    try {
        request.onreadystatechange = getInfoForSearchShipper;
        request.open("GET", link, true);
        request.send();
    } catch (e) {
        alert("Unable to connect to server");
...
function getInfoForSearchShipper() {
    if (request.readyState == 4) {
        let htmlSearchResult = request.responseText;
        document.getElementById("shipperHomeResult").innerHTML = htmlSearchResult;
    }
}
